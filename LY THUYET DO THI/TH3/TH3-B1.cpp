...
#include <stdio.h>
#include <stdlib.h>
#define INP "D:\\PROJECT VISUAL\\LY THUYET DO THI\\TH3\\TH3-BOT.inp"
#define OUT "D:\\PROJECT VISUAL\\LY THUYET DO THI\\TH3\\TH3-BOT.txt"
...
int main() {
   ...
   //for (int k = 0; k < n; k++)
    ...
    printf("== CHUONG TRINH TIM KIEM DUONG DI NGAN NHAT BANG THUAT TOAN DIJKSTRA ==\n");
  
    /* Do ta dang tinh toan tu dinh 0 nen
     muon hien thi tu dinh 1 thi can dung i + 1 de phu hop */
  
    printf("\n\t ==> DANG TIEN HANH TIM DUONG DI NGAN NHAT....  \n");
  
    if (Len[b] > 0 && Len[b] < sum) {
        fprintf(fo, "\t\t DUONG DI NGAN NHAT TU %d DEN %d: VOI TONG DO DAI NGAN NHAT LA: %d\n", a + 1, b + 1, Len[b]);
        ...
        fprintf(fo, "EROR 404. KHONG TIM THAY DUONG DI TU %d DEN %d\n", a + 1, b + 1);
    }
  
    printf("\t ==> DA HOAN TAT VIEC TIM DUONG DI NGAN NHAT!.\n");
  
    fclose(fi);
    fclose(fo);
  
    printf("==> HOAN THANH ! VUI LONG MO File *.txt DE XEM KET QUA. :)\n");
    return 0;
}
